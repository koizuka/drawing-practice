...
長時間（実測 150〜261 秒）の連続した高頻度の短ストローク連打。**間欠的で閾値は可変**（8 分以上描いても出ない回もある）。「2 秒以上止めずに描き続けた継続時間（streak）」が長いほど起きやすい。既知の WebKit 報告と同類:
...
注意: フリーズと「意図的な休憩」は、どちらもページから見ると「イベントの不在」で**原理的に区別できない**。そのため誤検出を完全には消せないが、(1) 長い streak の後に限定、(2) 非ブロッキングで入力再開時に即消灯、(3) 一定時間で自動消灯、により実用上無害なヒントに抑えている。
