...
- **Camera (pan/zoom)** is persisted as part of the draft. `SplitLayout` subscribes to `viewTransform` and tail-debounces a `flushVersion` bump after 250ms of stillness so the camera flushes once per gesture (not per frame). When the camera lands exactly at the registered home (`!viewTransform.isDirty()` — covers Cmd+0 / toolbar reset / viewer setHome on reference load), the debounce is bypassed and flush fires immediately. On restore, the camera is buffered in `pendingCameraRef` when the active source mounts a viewer that calls `setHome(0,0,1)` (image / url / pexels / youtube / sketchfab-fixed), and applied via `viewTransform.setCamera` from a parent effect keyed on `referenceSize` — runs after the viewer's setHome so the persisted camera wins. Sources without such a viewer apply the camera directly during `loadDraft`.
- **Discrete UI buttons flush immediately**, mirroring reference changes: collapse-toggle, flip-toggle, and any guide change (grid mode cycle, line add/remove/clear) bump `flushVersion` so a fast reload retains the explicit user intent.
