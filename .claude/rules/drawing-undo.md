...
**Timer interaction**: `DrawingPanel.handleClear` calls `timer.pause()` (NOT `timer.reset()`) on tentative clear so Undo restores the elapsed reading alongside the strokes. The reset happens at **stroke START**, not on commit: `DrawingPanel.handleStrokeStart` (wired to `DrawingCanvas`'s `onStrokeStart`, fired on pen-down) reads `strokeManager.isTentativeClearActive()` and, if active, calls `timer.reset()` then `timer.start()`. **Why pen-down rather than `endStroke`:** resetting/starting on release left the new opening stroke untimed (timer jumped to 0 only when the pen lifted); starting at pen-down times it. The tentative clear is still *committed* later by `endStroke` — only the timer bookkeeping moved earlier. Trade-off: if the pen-down stroke is then cancelled by escalating to a pinch (`cancelStroke`) while still tentative, the timer stays at 0/running and the pre-clear elapsed value is lost on a subsequent Undo — rare, the value was about to be discarded by the commit anyway, accepted (parallel to trace-score history not surviving Undo). `handleStrokeCountChange` no longer carries a `committedTentativeClear` flag; its `timer.start()` guard remains only as a safety net for non-pen commit paths (lasso-delete).
