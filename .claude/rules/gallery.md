...
- **Input freeze while the load is in flight** (`drawingLoadInFlight` → `DrawingCanvas.inputFrozen`): held from the button press until the strokes land — immediately for most loads, at the deferred migration for legacy records with a fresh reference. Without it, a stroke drawn during the async image-Blob lookup or before the migration's `loadState` would be silently wiped. Cleared defensively on `changeReference` / `resetReferenceOnError` (navigating away aborts the load).
- **Every gallery load cancels superseded pending restores**: `applyLoadedDrawingStrokes` nulls `pendingMigrationRef` / `pendingGalleryCameraRef` / `pendingCameraRef` up front. The referenced-load path clears them via `changeReference`, but a no-reference or evicted-reference record never calls it — a legacy autosave draft still waiting for its size callback would otherwise land its strokes/camera on top of the gallery load.
- **Trace scoring is reset on load** (`onTraceResetScores` in `DrawingPanel.executeLoadDrawing`): scores/attemptMap are not part of the record and would reference wiped strokes. The transient "Saved" indicator is cleared too. The overwrite confirm is a MUI `Dialog` (focus trap — keyboard must not reach the gallery behind it).
- **Deferred-migration gallery loads flush autosave on landing** (`skipUserStartGuard` branch in `handleReferenceImageSize`): the reference was already flushed with an empty stroke set when `changeReference` ran, so the stroke-landing flush narrows the reload-loses-the-drawing window to the image-load duration (accepted residual).
