...
`ImageViewer`, `TraceTemplateViewer` and `YouTubeViewer` all render `GuideState.masks` via `drawMasks` and support the `'mask'` `GuideInteractionMode` (drag → `onAddMask`, tap within the 5px drag threshold on a mask → `onRemoveMask`, two fingers cancel and pinch — same as `'add'`). **Draw order is fixed in every viewer: reference content → `drawMasks` → `drawGrid` → `drawGuideLines`** (then overlay strokes) — the grid stays visible over a hidden region as the position anchor. Masks are world-coord rects like guide lines (pan/zoom/flip for free, `shiftGuideState` migrates them), `masksHidden` (absent ≡ true) is persisted so a reload never reveals the answer, and they are reference-only: not drawn on `DrawingCanvas`, not in the stroke undo stack, and not part of exports / gallery thumbnails (those render stroke data only). SketchfabViewer (iframe, no overlay) and pose browse have no mask layer.
...
**Pose history**: every successful generation is appended to the `poseHistory` Dexie table (pose JSON + hint + small JPEG thumbnail Blob, cap 50, LRU by `lastUsedAt` — selecting an entry bumps it via `touchPoseHistory`). The thumbnail is captured synchronously in the commit `.then` — `refinePoseUntilValid` guarantees the final (= committed) candidate is the last one applied via `measurePose` (it re-measures after a rounds-exhausted changed refine), and `captureScreenshot` calls `vrm.update(0)` before rendering because `applyToVrm` writes NORMALIZED bones that reach the mesh only through `vrm.update` (normally the rAF loop — which has not ticked yet in this synchronous chain; without it the shot shows the previous pose). A not-yet-ready viewer just yields a thumbnail-less entry. The 履歴 button opens a Popover grid; selecting an entry commits a fresh `referenceInfo` (one undo entry) onto the CURRENT vrmId and deliberately does NOT mark it `selfCommitted`, so the hint-field re-seed and the in-flight-generation abort both fire — both are wanted when restoring a past pose.
