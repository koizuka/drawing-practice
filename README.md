...
- **ポーズお手本（AI）**: 棒人間を小さなスケッチパッドに描き、一言ヒント（任意）を添えて「ポーズ生成」すると、Claude API がポーズを解釈して 3D マネキン（VRM）に適用。マネキンは自由に回転・ズームでき、「この角度で固定」でスクリーンショットを手本化（グリッド・答え合わせ・ギャラリー対応）。モデルは同梱マネキンに加えて手持ちの .vrm ファイルも読み込み可能（ブラウザ内に保存）。API キーはユーザー各自の Anthropic キーをダイアログで設定（localStorage、ブラウザから直接 API を呼ぶ BYOK 方式）
