...
- **ポーズお手本（AI）**: 棒人間を小さなスケッチパッドに描くか、一言ヒントを書いて（どちらか一方でも、併用でも可）「ポーズ生成」すると、Claude API がポーズを解釈して 3D マネキン（VRM）に適用。適用結果は幾何的に自動検証され（床へのめり込み・浮遊・バランス・四肢の交差）、問題があれば AI が同じ会話の中で自動修正（最大2回、ボタン表示は「調整中…」）。「結跏趺坐」のような分かりやすいポーズならヒントの言葉だけでも生成可能。マネキンは自由に回転・ズームでき、「この角度で固定」でスクリーンショットを手本化（グリッド・答え合わせ・ギャラリー対応）。モデルは同梱マネキンに加えて手持ちの .vrm ファイルも読み込み可能（ブラウザ内に保存）。API キーはユーザー各自の Anthropic キーをダイアログで設定（localStorage、ブラウザから直接 API を呼ぶ BYOK 方式）
