...
export function referenceKey(info: ReferenceInfo): string {
  switch (info.source) {
    case 'sketchfab':
      return `sketchfab:${info.sketchfabUid}`;
    case 'image':
      return `image:${info.url ?? info.fileName}`;
    case 'url':
      return `url:${info.imageUrl}`;
    case 'youtube':
      return `youtube:${info.youtubeVideoId}`;
    case 'pexels':
      return `pexels:${info.pexelsPhotoId}`;
    case 'trace-template':
      return `trace-template:${info.templateId}`;
    ...
    case 'pose':
      return `pose:${info.vrmId}:${info.pose ? JSON.stringify(info.pose) : ''}`;
  }
}
