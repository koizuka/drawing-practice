...
Drawing Practice is a line-drawing practice tool for **iPad + Apple Pencil**. Users view a reference (Sketchfab 3D model, image file/URL, YouTube video, Pexels photo, a bundled trace template, or an AI-posed 3D mannequin) on one side and draw on the other, with synchronized grid and guide lines for alignment. Trace templates additionally score each stroke against a target shape and render the deviation as a red feedback overlay. The pose source ('pose') lets the user sketch a stick figure + one-line hint, calls the Anthropic Claude API directly from the browser (BYOK, like Pexels) to get a semantic pose JSON, and applies it to a three.js + @pixiv/three-vrm mannequin (bundled `public/mannequin.vrm` or a user-loaded .vrm persisted in Dexie `poseAssets`); "Fix This Angle" captures a screenshot into the standard fixed-image path. The pose domain logic lives in `src/pose/` (prompt, PoseJson sanitizing, joint mapping — three loads only inside the lazy `PoseSourcePanel` chunk).
