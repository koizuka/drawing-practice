...
Drawing Practice is a line-drawing practice tool for **iPad + Apple Pencil**. Users view a reference (Sketchfab 3D model, image file/URL, YouTube video, Pexels photo, a bundled trace template, or an AI-posed 3D mannequin) on one side and draw on the other, with synchronized grid and guide lines for alignment. Trace templates additionally score each stroke against a target shape and render the deviation as a red feedback overlay. The pose source ('pose') lets the user sketch a rough figure and/or type a one-line hint (either alone suffices; hint-only uses a text-only prompt variant), calls the Anthropic Claude API directly from the browser (BYOK, like Pexels) to get a semantic pose JSON, and applies it to a three.js + @pixiv/three-vrm mannequin (bundled `public/mannequin.vrm` or a user-loaded .vrm persisted in Dexie `poseAssets`). Limbs can be posed by angles OR by placement targets (`handAt`/`footAt`/`kneeAt`/`elbowAt`, plus `body.hipsHeight`) solved client-side with analytic two-bone IK (`src/pose/poseIk.ts`); the applied pose is then geometrically validated (floor penetration / floating / static balance / limb & head crossing / left-right mix-up — `src/pose/poseValidation.ts`) and implausibilities are fed back to the model for in-conversation correction, max 2 bounded rounds (`poseRefineLoop.ts`). "Fix This Angle" captures a screenshot into the standard fixed-image path. Every successful generation is also recorded in Dexie `poseHistory` (pose JSON + hint + thumbnail, cap 50, LRU) and can be re-applied to the current mannequin from the 履歴 popover in `PoseSourcePanel`. The pose domain logic lives in `src/pose/` (prompt, PoseJson sanitizing, joint mapping — three loads only inside the lazy `PoseSourcePanel` chunk). Prompt recipe coordinates MUST be machine-verified on the real mannequin's proportions via the headless harness (`poseTestHarness.ts` + fixture from `npm run extract-pose-rig`; a recipe the validator rejects burns refine rounds and makes the model wreck a correct pose) — see `docs/pose-headless-loop.md`, which also covers the memoized LLM-in-the-loop runner (`poseLlmLoop.manual.test.ts`, key in `.env.local`). Hair not hanging down in inverted poses (handstand etc.) is a spring-bone parameter trait of the VRM model itself, not a code bug — see `docs/vrm-springbone-gravity.md`.
...
- **Grid modes** (`GridMode`: none/normal/large/**perspective**) are picked from a popover (`GridModePopoverButton`, on the reference toolbar; stands in on the drawing toolbar only while the reference panel is collapsed). Perspective mode projects a floor+walls box (`src/guides/perspective.ts`, pure param→world-2D-segments function with dolly-zoom strength: apparent size stays put, only distortion changes) through the same `drawGrid` entry as the flat grids, so all four viewers render it with zero prop changes. Its yaw/pitch/strength live in `GridSettings.perspective` (persisted; anchor `centerX/Y` is a world coordinate, shifted by `shiftGuideState` in coord migration) and are edited via `PerspectiveController` — a floating widget on the drawing canvas (rotation pad + strength slider + place-anchor tap mode + reset). Mid-drag updates sync with `transient: true`, which SplitLayout routes to the 2s-debounce autosave path instead of the immediate flush; the pointer-up sync is non-transient and flushes (one save per gesture). The place-anchor mode is `GuideInteractionMode 'place-center'` on the reference viewers and a tap-intercept (no `startStroke`, so no timer start) on `DrawingCanvas`.
...
- **Apple Pencil input freeze**: resolved — WebKit/iPadOS suspends page-wide input delivery while rAF/main thread stay alive; see `docs/apple-pencil-input-freeze.md`. The shipped mitigation is `DrawingFreezeHint` (a hint shown after a long continuous-draw run goes silent; recovery = lift the pen ~2s). The `?diag=touch` diagnostics harness used for the investigation has been removed (recoverable from git history).
